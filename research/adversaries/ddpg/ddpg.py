...
class DDPGagent:
    ...
    def get_action(self, state):
        ...
        action = action.detach().numpy()[0,0] # due to batch dim
        return action    
    ...
    def update(self, batch_size):
        ...
        Qprime = rewards + self.gamma * next_Q # current reward + target Q
        ...
        for target_param, param in zip(self.actor_target.parameters(), self.actor.parameters()):
            target_param.data.copy_(param.data * self.tau + target_param.data * (1.0 - self.tau)) # just taking a portion of the update.
